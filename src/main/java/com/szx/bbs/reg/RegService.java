package com.szx.bbs.reg;

import java.util.Date;

import com.szx.bbs.common.authcode.AuthCodeService;
import com.szx.bbs.common.contants.AttributeConstant;
import com.szx.bbs.common.kit.EmailKit;
import com.szx.bbs.common.model.AuthCode;
import com.szx.bbs.common.model.User;
import com.szx.bbs.message.MessageService;
...
public class RegService
{
	...
	public Ret reg(String email, String userName, String password, String nickName, String ip) {
		...
		if (account.save()) 
		{
			...
			{
				return Ret.fail("msg", "注册成功，但是激活邮件发送失败，可能是邮件服务器出现故障，请稍后重试，多谢！");
			}
		} 
		else 
		{
	...
	private boolean sendRegActivateAuthEmail(String authCode, User reg) {
		String title = "双子星论坛激活";
		String content = "欢迎加入双子星论坛,在这里，你可以分享你的生活。在浏览器地址栏里输入并访问下面激活链接即可完成账户激活：\n\n"
	...
	public Ret activate(String authCodeId) {
		...
		if (authCode != null && authCode.isValidRegActivateAuthCode()) {
			...
			if (n > 0) {
				...
				return Ret.ok("msg", "账号激活成功，欢迎加入双子星论坛！");
			} else {
